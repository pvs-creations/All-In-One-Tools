    ...
    <div class="single-page-inner">
        <form method="POST">
            <div class="form-group">
                <label for="jsonBeautifier" class="custom-label">{{ trans('webtools/tools/json-beautifier.label') }}</label>
                <textarea type="text" name="jsonBeautifier" id="jsonBeautifier" class="custom-textarea" placeholder="{{ trans('webtools/tools/json-beautifier.placeholder') }}" rows="5" required>{{ request()->input('jsonBeautifier') }}</textarea>
                <input type="hidden" name="submit" value="true">
            </div>
            <button class="btn custom--btn button__lg">{{ trans('webtools/tools/json-beautifier.submit') }}</button>
            @if ($error == true)
                <div class="alert alert-danger rounded-pill mt-3 p-l-25 p-r-25 bg-danger border-0"><h5 class="m-0 text-light">{{ trans('webtools/tools/json-beautifier.invalid-msg') }}</h5></div>
            @endif
        </form>
        ...
        @if ($jsoncode)
            ...
            <div class="row">
                <div class="col-12">
                    <div class="form-group">
                        <label class="custom-label">{{ trans('webtools/tools/json-beautifier.converted-label') }}</label>
                        <div class="copy-textarea-btn">
                            <textarea type="email" id="json-copy" class="custom-textarea" rows="9">{{ $jsoncode }}</textarea>
                            <button onclick="window.writeClipboardTextVanilla(this, document.getElementById('json-copy').value)" class="btn custom--btn button__md copy-btn btn__dark">{{ trans('webtools/tools/json-beautifier.btn-copy') }}</button>
                        </div>
                    </div>
                </div>
            </div>
