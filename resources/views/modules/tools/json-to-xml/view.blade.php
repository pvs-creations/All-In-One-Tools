    ...
    <div class="single-page-inner">
        ...
        <form method="POST">
            <div class="form-group">
                <label for="jsonToXml" class="custom-label">{{ trans('webtools/tools/json-to-xml.label') }}</label>
                <textarea type="text" name="json_to_xml_data" id="jsonToXml" class="custom-textarea" placeholder="{{ trans('webtools/tools/json-to-xml.placeholder') }}" rows="5" required>{{ request()->input('json_to_xml_data') }}</textarea>
                <input type="hidden" name="submit" value="true">
            </div>
            <button class="btn custom--btn button__lg">{{ trans('webtools/tools/json-to-xml.submit') }}</button>
            @if ($error == true)
                <div class="alert alert-danger rounded-pill mt-3 p-l-25 p-r-25 bg-danger border-0"><h5 class="m-0 text-light">{{ trans('webtools/tools/json-to-xml.invalid-msg') }}</h5></div>
            @endif
        </form>
        @if ($jtx)
            <hr class="">
            ...
            <div class="row">
                <div class="col-12">
                    <div class="form-group">
                        <label class="custom-label">{{ trans('webtools/tools/json-to-xml.converted-label') }}</label>
                        <div class="copy-textarea-btn">
                            <textarea type="email" id="xml-copy" class="custom-textarea" placeholder="{{ trans('webtools/tools/json-to-xml.converted-textarea-ph') }}" rows="5">{{ $jtx }}</textarea>
                            <button onclick="window.writeClipboardTextVanilla(this, document.getElementById('xml-copy').value)" class="btn custom--btn button__md copy-btn btn__dark">{{ trans('webtools/tools/json-to-xml.btn-copy') }}</button>
                        </div>
                    </div>
                </div>
            </div>
