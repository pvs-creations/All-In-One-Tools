    ...
    <div class="single-page-inner">
        <form method="POST">
            <div class="form-group">
                <label for="text_content" class="custom-label">{{ trans('webtools/tools/paste-and-share-text.label_enter_text') }}</label>
                <textarea type="text" name="text_content" id="text_content" class="custom-textarea" placeholder="Enter Text" rows="8">{{ old('text_content') }}</textarea>
            </div>
            @error('text_content')
                <div class="alert alert-danger rounded-pill mt-3 p-l-25 p-r-25 bg-danger border-0"><h5 class="m-0 text-light">{{ $message }}</h5></div>
            @enderror
            @csrf
            <input type="hidden" name="submit" value="true">
            <button class="btn custom--btn button__lg">{{ trans('webtools/tools/paste-and-share-text.btn_Publish') }}</button>
        </form>
        ...
        @if ($created !== false)
            ...
            <div class="form-group">
                <label for="" class="custom-label">{{ trans('webtools/tools/paste-and-share-text.label_Share_Link') }}</label>
                <div class="copy-input">
                    <input type="text" id="decoded-copy" class="custom-input" value="{{ url('bins/' . $created->slug) }}">
                    <button onclick="window.writeClipboardTextVanilla(this, document.getElementById('decoded-copy').value)" class="btn custom--btn button__md copy-btn btn__dark">{{ trans('webtools/tools/paste-and-share-text.btn-copy') }}</button>
                </div>
            </div>
        @endif
