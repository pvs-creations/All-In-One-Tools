    ...
    <div class="single-page-inner">
        <form>
            <div class="form-group">
                <label for="decodeQuote" class="custom-label">{{ trans('webtools/tools/quoted-printable-decode.label') }}</label>
                <textarea type="text" name="decodeQuote" id="decodeQuote" class="custom-textarea" placeholder='{{ trans('webtools/tools/quoted-printable-decode.placeholder') }}' rows="5" required>{{ request()->input('decodeQuote') }}</textarea>
                <input type="hidden" name="submit" value="true">
            </div>
            <button class="btn custom--btn button__lg">{{ trans('webtools/tools/quoted-printable-decode.submit') }}</button>
            @if (request()->input('submit') && request()->input('decodeQuote') == '')
                <div class="alert alert-danger rounded-pill mt-3 p-l-25 p-r-25 bg-danger border-0"><h5 class="m-0 text-light">{{ trans('webtools/tools/quoted-printable-decode.fieldEmpty') }}</h5></div>
            @endif
        </form>

        @if ($decoded_printable)
            <hr class="">

            <div class="row">
                <div class="col-12">
                    <div class="form-group">
                        <label class="custom-label">{{ trans('webtools/tools/quoted-printable-decode.converted-label') }}</label>
                        <div class="copy-textarea-btn">
                            <textarea type="email" id="decoded-copy" class="custom-textarea" rows="9">{{ $quoted-printable-decode }}</textarea>
                            <button onclick="window.writeClipboardTextVanilla(this, document.getElementById('decoded-copy').value)" class="btn custom--btn button__md copy-btn btn__dark">{{ trans('webtools/tools/quoted-printable-decode.btn-copy') }}</button>
                        </div>
                    </div>
                </div>
            </div>
